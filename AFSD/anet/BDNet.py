...
class CoarsePyramid(nn.Module):
    def __init__(self, feat_channels=(832, 1024), frame_num=2304):
    ...
    def forward(self, feat_dict, ssl=False):
        ...
        for i, conv in enumerate(self.pyramids):
            if i == 0:
                x = conv(x1)
                x = x.squeeze(-1).squeeze(-1) #x = conv(x1).mean(dim=(3,4)) #[1, 512, 288]
            else:
                x = conv(x)
        ...
        for i, feat in enumerate(pyramid_feats):
            ...
            locs.append(self.loc_heads[i](self.loc_head(loc_feat)).view(batch_num, 2, -1).permute(0, 2, 1).contiguous() * fpn_strides[i])
            confs.append(self.conf_head(conf_feat).view(batch_num, num_classes, -1).permute(0, 2, 1).contiguous())
            ...
            loc_prop_feat,  loc_prop_feat_  = self.loc_proposal_branch(loc_feat, frame_level_feat,segments, frame_segments)
            conf_prop_feat, conf_prop_feat_ = self.conf_proposal_branch(conf_feat, frame_level_feat, segments, frame_segments)
            ...
            if i == 0:
                ...
                if ssl:
                    return trip
            prop_locs.append(self.prop_loc_head(loc_prop_feat).view(batch_num, 2, -1).permute(0, 2, 1).contiguous())
            prop_confs.append(self.prop_conf_head(conf_prop_feat).view(batch_num, num_classes, -1).permute(0, 2, 1).contiguous())
            centers.append(self.center_head(loc_prop_feat).view(batch_num, 1, -1).permute(0, 2, 1).contiguous())
        ...
        return loc, conf, prop_loc, prop_conf, center, priors, start, end, start_loc_prop, end_loc_prop, start_conf_prop, end_conf_prop

class BDNet(nn.Module):
    def __init__(self, in_channels=3, backbone_model=None, training=True, frame_num=2304):
