...
def video2npy(data_path, anno_path, save_path, sample_fps=10.0, resolution=112, export_video_info_path=None):
    ...
    for video_name in sorted(list(set(df['video'].values[:]))):
        ...
        if fps < sample_fps:
            raise ValueError('{}: sample fps {} is lower original fps {}'.format(video_name, sample_fps, fps))
        ...
        if cur_count != int(count):
            raise ValueError('{}: total count {} is not equal to video count {}'.format(video_name, cur_count, count))
    ...
    if export_video_info_path is not None:
        out_df = pd.DataFrame(video_infos, columns=['video', 'fps', 'sample_fps', 'count', 'sample_count'])
